...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
    ...
    if ((wtx.IsCoinBase() || wtx.IsCoinStake()) && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain()){
            strHTML += BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, (wtx.IsCoinStake() ? nUnmatured - nDebit : nUnmatured))
                    + " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";

            // fix coinStake netAmount
            if(wtx.IsCoinStake()){
    ...
    for (unsigned int k=0; k < wtx.vout.size(); ++k)
    {
        ...
        if (wtx.mapValue.count(cbuf) && !wtx.mapValue[cbuf].empty())
            strHTML += "<br><b>" + tr("Description") + ":</b> " + GUIUtil::HtmlEscape(wtx.mapValue[cbuf], true) + "<br>";
    ...
    if (wtx.IsCoinBase() || wtx.IsCoinStake())
    {
        if (wtx.IsInMainChain() && nCredit == 0)
