...
void ThreadFlushWalletDB(void* parg)
{
    ...
    RenameThread("artboomerang-wallet");
...
bool LoadNewWallet(const CWallet& wallet, const string& strDest)
{
    ...
    {
        {
            ...
            if (!bitdb.mapFileUseCount.count(wallet.strWalletFile) || bitdb.mapFileUseCount[wallet.strWalletFile] == 0)
            {
                ...
                int64_t now = GetTime();
                std::string backupFilename = strprintf("wallet.%" PRId64 "dat.backup", now);
                filesystem::path pathBackup = GetDataDir() / backupFilename;

                try {
#if BOOST_VERSION >= 104000
                    // copy to dest location current wallet file
                    filesystem::copy_file(pathSrc, pathBackup, filesystem::copy_option::overwrite_if_exists);
                    filesystem::remove(pathSrc);
                    filesystem::rename(pathDest, pathSrc);
#else
                    filesystem::copy_file(pathSrc, pathBackup);
                    filesystem::remove(pathSrc);
                    filesystem::rename(pathDest, pathSrc);
#endif
                    printf("copied wallet.dat from %s\n", pathDest.string().c_str());
                    return true;
                } catch(const filesystem::filesystem_error &e) {
                    printf("error copying wallet.dat from %s - %s\n", pathDest.string().c_str(), e.what());
                    return false;
                }
            }
        }
